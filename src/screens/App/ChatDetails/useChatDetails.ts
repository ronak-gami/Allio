...
export const useChatDetails = (
  ...
  const setReplyMessage = useCallback((msg: ChatMsg) => setReplyToMsg(msg), []);
  const clearReplyMessage = useCallback(() => setReplyToMsg(null), []);
  ...
  const blockUser = useCallback(async () => {
    ...
    if (isMounted.current) setIsBlockedByMe(true);
  ...
  const unblockUser = useCallback(async () => {
    ...
    if (isMounted.current) setIsBlockedByMe(false);
  ...
  const clearChat = useCallback(async () => {
    try {
      ...
      // if (isMounted.current) setClearTime({ toDate: () => now });
  ...
  const selectTheme = useCallback(
    async (fileKey: string | null) => {
      ...
      try {
        ...
        // if (isMounted.current) setloding(false);
        setloding(false);
      }
    },
    [relationId],
  ...
  const removeTheme = useCallback(async () => {
    try {
      ...
      if (isMounted.current) setThemeModalVisible(false);
    }
  }, [relationId]);

  const navigateToProfile = useCallback(
    () => navigation.navigate(HOME.Profile, { email: targetUser?.email }),
    [navigation, targetUser?.email],
  );
  ...
  const warmUpCurrentLocation = useCallback(async (): Promise<boolean> => {
    try {
  ...
  const openSystemLocationSettings = useCallback(() => {
  ...
  const ensureLocationReady = useCallback(
    async (silent = false): Promise<boolean> => {
      ...
      if (!granted) {
        if (!silent) {
          if (isMounted.current) setLocationPromptVisible(true);
        } else {
          if (isMounted.current) setLocationPromptVisible(false);
        }
        return false;
      }
    ...
    [askLocationPermission, warmUpCurrentLocation],
  );

  const retryLocationPreparation = useCallback(async () => {
    if (isMounted.current) setLocationPromptVisible(false);
  ...
  const openLocationFullModal = useCallback(
    () => setLocationFullVisible(true),
    [],
  );
  const closeLocationFullModal = useCallback(
    () => setLocationFullVisible(false),
    [],
  );
  ...
  const stopLiveLocationShare = useCallback(async () => {
    try {
  ...
  }, [liveShareIdMine, relationId, myEmail, targetUser?.email]);

  const startLiveLocationShare = useCallback(
    async (minutes = 15) => {
      try {
        if (!relationId) return;
        const ok = await ensureLocationReady(true);
  ...
  const deleteMessagesForMe = useCallback(async () => {
  ...
  const deleteMessagesForEveryone = useCallback(async () => {
    if (!relationId || selectedMessages.length === 0) return;
    const relationRef = firestore().collection('relation').doc(relationId);
    for (const msgId of selectedMessages) {
  ...
  }, [relationId, selectedMessages, clearSelectedMessages, closeActionModal]);
  ...
  const handleEditMessage = useCallback(async () => {
    if (!editMsgId || !editText.trim() || !relationId) return;
    const relationRef = firestore().collection('relation').doc(relationId);
    await relationRef
      .collection('messages')
      .doc(editMsgId)
