...
export const useChatDetails = (targetUser: any) => {
  ...
  const sendLiveLocationMessage = async (location: {
    latitude: number;
    longitude: number;
  }) => {
    try {
      ...
      showError('Failed to share live location');
    }
  };
  // Add the handleShareLocation function
  const handleShareLocation = () => {
    navigation.navigate(HOME.LocationPickerScreen, {
      onSendLocation: (location: { latitude: number; longitude: number }) => {
        sendLocationMessage(location);
      },
    });
  };

  // Add the sendLocationMessage function
  const sendLocationMessage = async (location: {
    ...
    try {
      ...
      const data = {
        ...
        body: `${email1} has shared a location with you.`,
      };
